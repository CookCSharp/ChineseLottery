...
namespace UnionLotto
{
    public class History
    {
        ...
        //所有开奖号码
        private const string AllURL = "https://www.cwl.gov.cn/cwl_admin/front/cwlkj/search/kjxx/findDrawNotice?name=ssq&pageNo=1&pageSize=30";
        //部分开奖号码
        private const string URL = "https://www.cwl.gov.cn/cwl_admin/front/cwlkj/search/kjxx/findDrawNotice?name=ssq&issueCount=&issueStart=&issueEnd=&dayStart=&dayEnd=&pageNo=1&pageSize=30&week=&systemType=PC";

        private Dictionary<string, IList<int>> HistoryData = new Dictionary<string, IList<int>>();
        private Dictionary<string, IList<int>> HistoryRedData = new Dictionary<string, IList<int>>();
        ...
        public async void HttpGet(int pageSize = 50)
        {
            ...
            for (int i = 0; i < dataList!.AsArray().Count; i++)
            {
                ...
                HistoryRedData.Add(key, value.ToList());
                value.Add(int.Parse(dataList[i]!["blue"]!.ToString()));
                HistoryData.Add(key, value);
            }
            HistoryRedData = new Dictionary<string, IList<int>>(HistoryRedData.Reverse());
        ...
        public void GetData(int pageSize, out Dictionary<string, IList<int>> historyRedData, out Dictionary<string, IList<int>> historyData)
        {
            if (!_isStartGuess)
            {
                historyRedData = new Dictionary<string, IList<int>>(HistoryRedData.Take(pageSize).Reverse());
                historyData = new Dictionary<string, IList<int>>(HistoryData.Take(pageSize).Reverse());
            }
            else
            {
                historyRedData = new Dictionary<string, IList<int>>(HistoryRedData.Skip(1).Take(pageSize).Reverse());
